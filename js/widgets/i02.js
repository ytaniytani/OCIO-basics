...
const QUESTIONS = [
  {
    ...
    choices: [
      ...
      { v: '186', ok: true, why: 'これが正解です。半分の光でも、数値は 73% までしか下がりません。' },
      { v: '220', ok: false, why: '下がりかたが小さすぎます。' },
      { v: '64', ok: false, why: '4分の1にあたる数字です。下がりすぎです。' },
    ],
    after: '光は半分(50%)。でも数値は 186 で、もとの 73% です。',
  },
  {
    q: 'では逆に、写真の数値が 128(ちょうどまん中)のとき、光の量はもとの何%でしょう。',
    hint: '数値のまん中 = 光のまん中、でしょうか。',
    choices: [
      { v: '50%', ok: false, why: '多くの人がこう答えます。でも実際はもっとずっと暗い光です。' },
      { v: '35%', ok: false, why: 'まだ明るすぎます。' },
      { v: '21%', ok: true, why: 'これが正解です。数値のまん中は、光の量では5分の1くらいしかありません。' },
      { v: '75%', ok: false, why: '逆に明るくなっています。' },
    ],
    after: '数値 128 は、光の量にすると 21.4% です。',
  },
];
...
export default function i02(mount) {
  const w = createWidget(mount, {
    ...
    aim: '答えを予想してから確かめます。まちがえても大丈夫です。多くの人が同じところでつまずきます。',
  });
  ...
  function makeProof() {
    const rows = [1, 0.5, 0.25, 0.214, 0.125].map((lin) => {
  ...
  w.setDetails(`
    <p>この曲げかたには理由があります。人の目は暗いところの差にとても敏感で、
       明るいところの差にはにぶいからです。</p>
    <p>数値を素直に光の量どおりにすると、8bit(256段階)では暗い部分の段階が足りず、
